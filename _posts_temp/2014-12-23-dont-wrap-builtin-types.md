---
layout: post
title:  "Don't wrap built-in types"
...
Hace unas semanas llegué a la siguiente pregunta hecha en Stackoverflow

    http://stackoverflow.com/questions/5952287/why-is-this-java-code-6x-faster-than-the-identical-c-sharp-code


El autor de la pregunta tiene que resolver cierto problema escribiendo una funcion que es casi identica en codigo Java y C#.
Él afirma que existe una diferencia de rendimiento de ~6x a favor de Java. No comprende por qué tan abuldata diferencia a igual código.

Aqui el codigo escrito por el autor. Hice unas minimas modificaciones, eliminé el codigo que sirve para medir los tiempos, ya que es diferente en Java y en C#.
La siguiente funcion es válida tanto en Java como en C# (incluso es código C válido).

{% highlight cpp %}
int ProjectEulerProblem005b()
{
    int i = 20;
    while (true) {
        if ((i % 19 == 0) &&
...
No quiero ponerme a analizar si el codigo es correcto o si puede ser mejorado, sino, analizar el porqué de la diferencia en performance.


¿Por que?


Algunas personas dieron sus sugerencias aconsejando algunas cosas obvias:

    - In .Net, use the Stopwatch class for measurements (http://msdn.microsoft.com/en-us/library/system.diagnostics.stopwatch%28v=vs.110%29.aspx)
    - In Java, use System.nanoTime();
    - Take the average time of N executions (with N like 10.000, 100.000, ... )
    - Run using a Release full-optimized build
    - Console (Release build) executions for .Net
    - Ensure that you are comparing the same thing.


    - Measure only what matters
    - Use the same machine to perform measurements (or similar machines)
